...
# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/02b_aisweb.ipynb.
...
class AisWeb:
	...
	def _check_ils_dme(self, df):
		if (columns := {'freq', 'lat', 'lng', 'thr', 'ident'}).issubset(df.columns):
			...
			df.loc[idx, 'Latitude'] = df.loc[idx, 'lat'].apply(lambda x: convert_latitude(x))
			df.loc[idx, 'Longitude'] = df.loc[idx, 'lng'].apply(lambda x: convert_longitude(x))
			df.loc[idx, 'Entidade'] = (
				df.loc[idx, 'Entidade'] + ' ' + df.loc[idx, 'thr'] + ' ' + df.loc[idx, 'ident']
			)
	...
	def records(
		...
		df['Fonte'] = 'AISWEB'
		return map_channels(df, 'AISWEB').drop_duplicates(UNIQUE_COLS, ignore_index=True)


# %% ../../nbs/02b_aisweb.ipynb 9
