...
def merge_on_frequency(
    ...
    pp(
        "[bold green]Logging:[/bold green] [italic]Calculando distância entre estações mescladas."
    )
    df.loc[both, "Distance"] = df.loc[both, both_columns].progress_apply(
        get_km_distance, axis=1
    )
    ...
    assert pd.merge(df_both_left, df_both_right, how="inner", on=merge_cols).empty, (
        "Check merging steps, df_both_left and df_both_right should be disjoint"
    )
    ...
    assert (df_both.Distance > MAX_DIST).all(), (
        "Check merging steps, validation failed!"
    )
