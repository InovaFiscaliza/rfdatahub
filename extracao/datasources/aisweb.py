...
def convert_latitude(
    ...
    return str(
        multiplier * (float(lat[:2]) + float(lat[2:4]) / 60 + float(lat[5:7]) / 3600.0)
    )
...
def convert_longitude(
    ...
    return str(
        multiplier * (float(lon[:3]) + float(lon[3:5]) / 60 + float(lon[6:8]) / 3600.0)
    )
...
class AisWeb:
    ...
    def request_stations(
        ...
        try:
            ...
            return (
                self._process_data(dict_data)
                if dict_data.get("aisweb")
                else pd.DataFrame()
            )
        except ExpatError:
